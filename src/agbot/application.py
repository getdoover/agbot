...
from .app_state import AgbotState

log = logging.getLogger()

class AgbotApplication(Application):
    config: AgbotConfig  # not necessary, but helps your IDE provide autocomplete!

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.started: float = time.time()
        self.ui: AgbotUI = None
        self.state: AgbotState = None

    async def setup(self):
        self.ui = AgbotUI()
        self.state = AgbotState()
